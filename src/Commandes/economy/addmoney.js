...
module.exports = {
    ...
    name: "addmoney",
    description: "Commande administrative pour donner de l'argent",
    category: "economie",
    ownerOnly: false,
    crownOnly: true,
    ...
    async execute(interaction, client, db, emoji, color) {
        ...
        db.query(`SELECT * FROM money WHERE guildID = ? AND memberID = ?`, [interaction.guild.id, interaction.member.id], async (err, req) => {
            ...
            if(req.length > 1){
                nb2 = req[0].money;
                if(parseInt(nb) + parseInt(nb2)){
                    return interaction.reply(`Vous ne pouvez pas donner autant d'argent !`)
                }
            }else{
                if(nb > 2147483647){
                    return interaction.reply(`Vous ne pouvez pas donner plus de 2147483647 <:xenos_money:1021001754558615623>`)
                }
            }
