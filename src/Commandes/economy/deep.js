...
module.exports = {
	...
	name: "deep",
	description: "Envoyez votre argent en toute sécurité !",
	category: "economie",
	ownerOnly: false,
	fondateurOnly: false,
	...
	async execute(interaction, client, db, emoji, color) {
        ...
        const id = await functionID("deep")
        ...
        db.query(`SELECT * FROM banque WHERE guildID = ? AND memberID = ?`, [interaction.guild.id, interaction.member.id], async (err, req) => {
            if(amount === "all") amount = cash[0].money;
            if(cash.length < 1 || amount < 1){
                return interaction.reply(`Vous n'avez pas d'argent !`)
            }else if(cash[0].money < amount){
                const thune = cash[0].money < 1 ? 0 : cash[0].money;
                return interaction.reply(`Vous n'avez pas assez d'argent à déposer dans votre banque ! Il vous manque ${numStr(amount - thune)} <:xenos_money:1021001754558615623>`)
            }else if(req.length < 1){
                db.query(`INSERT INTO banque (memberID, money, guildID, id) VALUES (?, ?, ?, ?)`, [interaction.member.id, amount, interaction.guild.id, id])
                functionRemoveMoney(interaction.member.id, interaction.guild.id, amount)
                return interaction.reply(`Vous avez déposé ${numStr(amount)} <:xenos_money:1021001754558615623> dans votre banque.`)
            }else{
                db.query(`UPDATE banque SET money = ? WHERE guildID = ? AND memberID = ?`, [(parseInt(req[0].money) + parseInt(amount)), interaction.guild.id, interaction.member.id])
                functionRemoveMoney(interaction.member.id, interaction.guild.id, amount)
                return interaction.reply(`Vous avez déposé ${numStr(amount)} <:xenos_money:1021001754558615623> dans votre banque.`)
            }
        })
